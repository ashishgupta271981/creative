...
class MY_Controller extends CI_Controller
{
    ...
    public function utility_format_ids_integer(&$item,$key,$tocheck=null)
	{
        ...
        if($tocheck !=null)
        {
            ...
            {
                if((gettype($item)=='array') and  (isset($item[$key])))
                {
                    $item[$key] = (int) $item[$key];
                }
                elseif((gettype($item)=='object') and  (isset($item->{$key})))
                {
...
class Api_Controller extends MY_Controller
{
    ...
    */
	...
	function __construct($config = 'rest')
	{
        ...
        $language_slug = $_SESSION['set_language'];
        $this->lang->load('api_messages', strtolower($this->langs[$language_slug]['name']));
       ...
       */
       ...
       // Initialise the response, request and rest objects
        ...
        $dp_configs = $this->config->item('profile_dp');
        $cover_configs = $this->config->item('profile_cover');

        if(empty($this->user_info->dp_image) || $this->user_info->dp_image == NULL){
            $this->user_info->dp_image ="default_dp.jpg";
        }
        if(empty($this->user_info->cover_image) || $this->user_info->cover_image == NULL){
            $this->user_info->cover_image ="default_cover.jpg";
        }

        $this->user_info->dp_image = $dp_configs['path'] . $this->user_info->dp_image;
        $this->user_info->cover_image = $cover_configs['path'] . $this->user_info->cover_image;
        unset($this->user_info->ip_address);
        unset($this->user_info->password);
        unset($this->user_info->salt);
        unset($this->user_info->activation_code);
        unset($this->user_info->forgotten_password_code);
        unset($this->user_info->forgotten_password_time);
        unset($this->user_info->remember_code);
        unset($this->user_info->updated_at);
	    unset($this->user_info->deleted_at);
	    unset($this->user_info->updated_by);
        unset($this->user_info->deleted_by);
        $this->load->model('city_translation_model');
        $this->load->model('state_translation_model');
        $this->load->model('country_translation_model');
        $this->user_info->city = $this->city_translation_model->getCityNameById($this->user_info->city_id);
        $this->user_info->state = $this->state_translation_model->getStateNameById($this->user_info->state_id);
        $this->user_info->country = $this->country_translation_model->getCountryNameById($this->user_info->country_id);
        $this->api_response = array('essentials' => $this->user_info);
        $seo['title'] = 'Creative Freelancers INC - the home of virtual office  & more | CFI';
        $seo['description'] = 'CFI is the leading marketplace for freelance work.Crowdsource your project at #1 freelance platform. Over 50,000 Happy Clients.';
        $seo['keywords'] = 'cfi, creative freelancers inc, creative,freelancers,creative freelancers';
        $this->user_info->seo = $seo;
...
/*
    ...
    public function genrate_pages($records_per_page,$total_records, $current_page)
    {
        //$records_per_page = $this->config->item('RPP_project_search');//RPP_portfolio_project_list
        $total_pages = (int) ($total_records / $records_per_page);
        if(($total_records % $records_per_page) != 0) $total_pages++;
        if($total_pages < 2) return '';
        $str_pages ='';
        for($_i=1;$_i<=$total_pages;$_i++)
        {
            if($_i == $current_page)
            {
                $str_pages .= '<li class="active"><a href="javascript:void(0);">'.$_i.'</a></li>';
            }
            else
            {
                $str_pages .= '<li><a href="javascript:void(0);" ng-click="getResult('.$_i.');">'.$_i.'</a></li>';
            }
        }
        return $str_pages;
    }
