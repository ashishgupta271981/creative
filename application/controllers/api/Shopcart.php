 ...
 *
 */
class Shopcart extends Api_Controller
{
	/**
	...
	/**
	 * To get saved as wish list projects
	 *
	 * @Controller  Shopcart
	 * @method    	wish-list [via GET]
	 * @param	page ID of type integer
	 * @author      Deepak Patil
	 * @created_at  Dec 29,2016
	 * @purpose 	Its shows the freelancer wish-listed projects
	 **/
	public function wish_list_get($pid=1)
	{
		try
		{
			...
			$total_records = $this->project_model->get_wish_listed_total_records($this->user_id);
			if(!$total_records)
			{
				$lang_key = 'no_access';
				throw new Exception($this->lang->line($lang_key,$this->config->item($lang_key)));
			...
			$records_per_page = $this->config->item('RPP_wish_listed');
			$this->api_response['pages'] = $this->genrate_pages($records_per_page,$total_records,$pid);

			$limit = ($pid-1) * $records_per_page;
			/*Prepare response*/
			$this->api_response['response'] = $this->project_model->get_wish_listed($this->user_id,$records_per_page,$limit);
