...
class Projectowners extends Api_Controller
{
	...
	function __construct()
	{
		...
		$this->methods['users_get']['limit'] = 360000; // 1000 requests per second per user/key
		$this->methods['users_post']['limit'] = 360000; // 1000 requests per second per user/key
		$this->methods['users_delete']['limit'] = 360000; // 1000 requests per second per user/key
	...
	public function projects_get()
       {
           ...
           if ($id === NULL)
           {
               ...
               if ($users)
               {
                   ...
                   $this->response($users, API_Controller::HTTP_OK); // OK (200) being the HTTP response code
               }
               else
               {
                   // Set the response and exit
                   ...
                   ], API_Controller::HTTP_NOT_FOUND); // NOT_FOUND (404) being the HTTP response code
               }
           }
           ...
           if ($id <= 0)
           {
               ...
               $this->response(NULL, API_Controller::HTTP_BAD_REQUEST); // BAD_REQUEST (400) being the HTTP response code
           }
           ...
           if (!empty($user))
           {
               $this->set_response($user, API_Controller::HTTP_OK); // OK (200) being the HTTP response code
           }
           else
           {
               $this->set_response([
