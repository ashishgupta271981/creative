...
class Orders extends Api_Controller
{
	 ...
	 **/
	public function transactions_get($pid=1, $sort_on)
	{
		try
		{
			/*Load User Model*/
			$this->load->model('order_model');
			$total_records = $this->order_model->get_total_transactions($this->user_id);
			if(!$total_records)
			{
				$lang_key = 'no_access';
				throw new Exception($this->lang->line($lang_key,$this->config->item($lang_key)));
			...
			$records_per_page = $this->config->item('RPP_history');
			$this->api_response['pages'] = $this->genrate_pages($records_per_page,$total_records,$pid);

			$limit = ($pid-1) * $records_per_page;
			/*Prepare response*/
			$this->api_response['response'] = $this->order_model->get_transactions($this->user_id,$sort_on,$records_per_page,$limit);
